...
- Sixteen addressable status LEDs in one chain: DIN = GPIO43. On startup they run one quick rainbow animation, then each Hall pad controls the matching LED; idle LEDs glow at 30% in the current MIDI channel color and the pressed pad LED rises to 100% until release. Channel colors are CH1 blue, CH2 red, CH3 green, and CH4 purple.
